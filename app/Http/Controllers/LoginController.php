...
class LoginController extends Controller
{
    ...
    {
        $getUserDevice_token =Users::select("device_token")->where("id",$id)->first();
        if(!empty($getUserDevice_token->device_token))
        {

          $title = "Registered";
          $message = "Your account is successfully registered, please verify your email address & wait for account approval from admin end ";
          
          $this->sendNotification($title,$message,$getUserDevice_token->device_token);
                      $result = array('status'=> true, 'message'=>'OTP not Send');
...
//                                    DB::table('users')->insert($updateData);
                                   // DB::table('temp_users')->where('id',$temp_users->id)->delete();
                                      ...
                                      $getuser = Users::select("device_token")->where('email',$temp_users->email)->first();
                                     if(!empty($getuser->device_token))
                                     {
                                  //   dd($getuser->device_token);
                                
                                     //    $title = "Registered";
                                     //    $message = "Your account is successfully registered, please verify your email address & wait for account approval from admin end ";
                                      
                                        
                                     // $this->sendNotification($title,$message,$getuser->device_token);   
                                        
                                     }
                                        
                                     $data = Users::where('email',$email)->update(['email_verified_at'=>$date1]);
                                     $getdatalast = Users::where('email',$email)->first();

                                     $result=array('status'=>true,'message'=>'verify succesfully','last_register_id'=>$getdatalast->id);                   
     ...
     // dd($request->input());
        ...
        if(!is_null($business))
        {
            ...
            else
            {
                ...
                $business  =  Users::where($where)->first();
             //  dd($business->status);
             Users::where('id',$business->id)->update(array('device_token'=>$request->device_token));

             if($business->status==2)
                {
         

                    Users::where($where)->update(array('login_status'=>1));
...
//        echo json_encode($result);
    ...
    public function myaccount(Request $request){
        $user_id = session()->get('id');
          ...
          $selectbusiness_categoryArray =explode(",",$account->business_category);
          $selectbusiness_sub_categoryArray =explode(",",$account->business_sub_category);
          //  dd($business_sub_categoryArray);
           $categorylist =  Categorys::where('status',0)->get();
           $subcategorylist =  SubCategorys::where('status',0)->get();
        //   dd($account->opeing_hour);
           $account->opeing_hour = isset($account->opeing_hour) ? $account->opeing_hour :"00:00";
           $account->closing_hour = isset($account->closing_hour) ? $account->closing_hour :"00:00";
          
           //dd(BusinessReviews::where('business_id',$user_id)->avg('ratting'));

          // $account->avgratting = number_format(BusinessReviews::where('business_id', $user_id)->avg('ratting'), 1);
     //   dd(BusinessReviews::where('business_id',$user_id));
    $account->ratting=    number_format(BusinessReviews::where('business_id', $user_id)->avg('ratting'), 1);
        

            $country_codedata =DB::table('country_codes')->get();
        return view('wemarkthespot.my-account',compact('account','categorylist','subcategorylist','country_codedata','selectbusiness_categoryArray','selectbusiness_sub_categoryArray'));
    }
        ...
        if($validate->fails()){
          ...
          //  $user = new Users();
            ...
            $fileimage="";
             ...
             $data['business_category']=isset($request->business_category)? implode(",",$request->business_category):$user->business_category;
             if(isset($request->hablamos_espanol) && $request->hablamos_espanol == 'on'){
                $data['hablamos_espanol']= 1;
             }else{
                $data['hablamos_espanol']= 0;
             }
             if(isset($request->religious_spiritual) && $request->religious_spiritual == 'on'){
                $data['religious_spiritual']= 1;
             }else{
                $data['religious_spiritual']= 0;
             }
             
             //Current promotion
             $data['business_sub_category']=isset($request->business_sub_category)? implode(",",$request->business_sub_category):$user->business_sub_category;
    ...
    {
            $user_id = session()->get('id');
        User::where('id',$user_id)->update(array('login_status'=>0));
        Session::flush();
    ...
    public function acchangepassword()
    {
        $user_id = session()->get('id');
        $account = Users::where('id', $user_id)->first();
        return view('wemarkthespot.ac-change-password',compact('account'));
    }

    public function businessuserchangepsd(Request $request)
    {
      //  dd($request->all());
        if(!empty($request->input()))
        {
    ...
    public function webpayment()
    {
        $user_id = session()->get('id');
        $account = Users::where('id', $user_id)->first();
        return view('wemarkthespot.webpayment',compact('account'));
    }
    ...
    //     return view('wemarkthespot.my-subscription',compact('account'));
    // }
    ...
    public function mysubscription(Request $request){
        ...
        $payment_data = payments::where('user_id', $user_id)->where('startDate', '>=', date("m/d/Y"))->orderBy('id', 'ASC')->select('plan_name','startDate','endDate')->get();
          
        return view('wemarkthespot.my-subscription',compact('account', 'payment_data'));
    }
    ...
    public function notifications(Request $request){
        ...
        return view('wemarkthespot.notifications',compact('account'));
    }
    ...
    public function testnotification()
    {
        ...
        $body ="testbody"; //$request->body;
        $deviceToken="dFmCC2cfH6HzeWr0xQ8hCA:APA91bE1YeoIq2_c4GH1KA5Lhhrgq0f7SzOIzqeuohriNodyJwQoeSFlqUtCpO57uWOLA4652L-vDw8g-5FBPhYS5eEKCiUMoGwWH93tgDfJVPVev0PDJn_5Y55mADijUL2Y_P4fycH4";   
