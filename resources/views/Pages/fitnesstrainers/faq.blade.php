			...
			<div class="col-12">
				<div class="card">
					<div class="border-bottom title-part-padding d-flex justify-content-between">
						...
						<a href="{{ url('/add_Faq') }}" class="btn btn-info btn-sm">
							Add FAQ
						</a>               
					</div>
					<div class="card-body">
								...
								@foreach ($faq as $user) 
									...
									<tr>
										...
										<td style="display: table-cell;">
												<span class="ml-2">{{ $user->question }}</span>
										</td>

										<td><textarea readonly class="form-control" style="min-height:130px; min-width:251px;">{{ $user->answers }}</textarea></td>
										
												<td>{{ $user->created_at }}</td>
										<td>
											<div class="table_action">
											 
												<a href="{{ route('Faq_delete',$user->id) }}" class="btn  btn-danger btn-sm list_delete " onclick="return confirm('Are you sure delete this FAQ？')">
													<i class="mdi mdi-delete"></i>
												</a> 
												
												<a style="display: " href="{{ url('Faq_edit',$user->id) }}" class="btn btn-info btn-sm list_edit">
													<i class="mdi mdi-lead-pencil"></i>
												</a> 
												<span class="status" style="display:none">
													<label class="switch">

														<input data-id="{{$user->id}}" class=" Faq_statuss switch-input" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="InActive" {{ $user->status ? 'checked' : '' }}>
